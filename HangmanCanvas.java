...
public class HangmanCanvas extends GCanvas {
	...
	/*
	 * GLabels that will display the word (initially with hyphens representing
	 * words which will updated with correct character guesses) and
	 * displayGuesses will display incorrect guesses
	 *
	 */
	...
	private void addSpine() {
		GLine spine = new GLine(getWidth() / 2, yCoordinateForHead + HEAD_RADIUS * 2, getWidth() / 2,
				yCoordinateForHead + HEAD_RADIUS + BODY_LENGTH);
		add(spine);

	}

	private void addRightArm() {
		GLine rightArm = new GLine(getWidth() / 2, yCoordinateForUpperArm, getWidth() / 2 + UPPER_ARM_LENGTH,
				yCoordinateForUpperArm);
		add(rightArm);

	}

	private void addLeftArm() {
		GLine leftArm = new GLine(getWidth() / 2, yCoordinateForUpperArm, getWidth() / 2 - UPPER_ARM_LENGTH,
				yCoordinateForUpperArm);
		add(leftArm);
	...
	private void addRightLeg() {
		...
		GLine rightLeg = new GLine(getWidth() / 2 + HIP_WIDTH, yCoordinateForHip, getWidth() / 2 + HIP_WIDTH,
				yCoordinateForFoot);
		add(rightHip);
		add(rightLeg);
	...
	private void addLeftLeg() {
		...
		add(leftHip);
		GLine leftLeg = new GLine(getWidth() / 2 - HIP_WIDTH, yCoordinateForHip, getWidth() / 2 - HIP_WIDTH,
				yCoordinateForFoot);
		add(leftLeg);
	}

	private void addRightHand() {
		GLine rightHand = new GLine(getWidth() / 2 + UPPER_ARM_LENGTH, yCoordinateForUpperArm,
				getWidth() / 2 + UPPER_ARM_LENGTH, yCoordinateForUpperArm + LOWER_ARM_LENGTH);
		add(rightHand);

	}

	private void addLeftHand() {
		GLine leftHand = new GLine(getWidth() / 2 - UPPER_ARM_LENGTH, yCoordinateForUpperArm,
				getWidth() / 2 - UPPER_ARM_LENGTH, yCoordinateForUpperArm + LOWER_ARM_LENGTH);
		add(leftHand);
	...
	private void addRightFoot() {
		GLine rightFoot = new GLine(getWidth() / 2 + HIP_WIDTH, yCoordinateForFoot,
				getWidth() / 2 + HIP_WIDTH + FOOT_LENGTH, yCoordinateForFoot);
		add(rightFoot);
	}

	private void addLeftFoot() {
		GLine leftFoot = new GLine(getWidth() / 2 - HIP_WIDTH, yCoordinateForFoot,
				getWidth() / 2 - HIP_WIDTH - FOOT_LENGTH, yCoordinateForFoot);
		add(leftFoot);
	}
